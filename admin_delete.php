...
<body>
	...
	<div id="omot">
		...
		<div id="meni">
			...
			<h4 align="center">Brisanje sadržaja</h4>
		<div id="admin_sredina1">
			<?php
				include("konekcija.inc");
				if(isset($_REQUEST['btnBrisi'])){
					$id_proizvod = $_REQUEST['brisanje'];
					foreach($id_proizvod as $id){
				...
				echo "<form><table border = '1px'><tr>
							<th class='paddingTH'>RB</th>
							<th class='paddingTH'>Naziv</th>
							<th class='paddingTH'>Cena</th>
							<th class='paddingTH'>Slika</th>
							<th class='paddingTH'>Brisi</th></tr>";
					$i = 1;
					if(mysqli_num_rows($rez) == 0){
						...
						while($r = mysqli_fetch_array($rez)){
							echo "<tr>
									...
									<td class='center'>".$r['naziv_proizvod']."</td>
									<td class='center'>".$r['cena_proizvod']."</td>
									<td class='center'><img src='".$r['putanja']."' width = '150px' height = '150px'/></td>
									<td class='center'><input type = 'checkbox' name = 'brisanje[]' value = '".$r['id_proizvod']."'/></td>
								  </tr>";
							$i++;
						}
					}
					echo " <tr><td colspan ='8'><center><input type = 'submit' name='btnBrisi' id='btnBrisi' value='Obrisi'/></center></td></tr>
						   </table></form>";
